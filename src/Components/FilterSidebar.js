// components/FilterSidebar.js - Fixed version
"use client";
  ...
  const fetchFilterOptions = async () => {
    try {
      ...
      setAvailableFilters({
        ...
        features: ['Low Power', 'High Speed', 'High Efficiency', 'Automotive', 'Industrial', 'Temperature Stable', 'Fast Switching', 'High Reliability']
      });
    } catch (error) {
      ...
      setAvailableFilters({
        brands: [
          { _id: 'texas-instruments', brandName: 'Texas Instruments', count: 5 },
          { _id: 'microchip-technology', brandName: 'Microchip Technology', count: 3 },
          { _id: 'stmicroelectronics', brandName: 'STMicroelectronics', count: 4 },
          { _id: 'intel', brandName: 'Intel', count: 2 },
          { _id: 'analog-devices', brandName: 'Analog Devices', count: 1 }
        ],
        manufacturers: [
          { _id: 'tsmc', manufacturerName: 'TSMC', count: 8 },
          { _id: 'globalfoundries', manufacturerName: 'GlobalFoundries', count: 3 },
          { _id: 'samsung', manufacturerName: 'Samsung', count: 2 },
          { _id: 'intel', manufacturerName: 'Intel', count: 2 }
        ],
        features: ['Low Power', 'High Speed', 'High Efficiency', 'Automotive', 'Industrial']
      });
    } finally {
            ...
            {itemsArray.length > 0 ? (
              itemsArray.map((item) => {
                ...
                const itemName = typeof item === 'object' 
                  ? (item.brandName || item.manufacturerName || item._id) 
                  : item;
                const itemCount = typeof item === 'object' ? item.count : null;
                
                return (
                  <label key={itemId} className="flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                      checked={filters[filterKey]?.includes(itemId) || false}
                      onChange={(e) => handleFilterChange(filterKey, itemId, e.target.checked)}
                    />
                    <span className="ml-2 text-sm text-gray-700">
                      {itemName}
                      {itemCount && <span className="text-gray-500 ml-1">({itemCount})</span>}
                    </span>
                  </label>
                );
              })
      ...
      {error && (
        <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-md">
          <p className="text-sm text-yellow-800">
            Using cached filters. Some options may be limited.
          </p>
        </div>
      )}
